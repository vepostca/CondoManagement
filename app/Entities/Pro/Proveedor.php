<?php
namespace InnovaCondomi\Entities\Pro;

use InnovaCondomi\Entities\Base\Identifiable as Model;
use Illuminate\Database\Eloquent\SoftDeletes;

/**
 * Class Proveedor
 * @package InnovaCondomi\Entities\Pro
 */
class Proveedor extends Model
{
    use SoftDeletes;
    public $table = 'pro_proveedor';
    protected $dates = ['deleted_at', 'created_at', 'updated_at'];


    public $fillable = [
        'com_cliente_id',
        'com_org_id',
        'codigo',
        'nombre',
        'id_legal',
        'siglas',
        'pro_actividad_proveedor_id',
        'observaciones',
        'activo'
    ];

    /**
     * The attributes that should be casted to native types.
     *
     * @var array
     */
    protected $casts = [
        'codigo' => 'string',
        'nombre' => 'string',
        'id_legal' => 'string',
        'siglas' => 'string',
        'observaciones' => 'string',
        'activo' => 'boolean',
        'created_ip' => 'string',
        'updated_ip' => 'string'
    ];

    /**
     * Validation rules
     *
     * @var array
     */
    public static $rules = [
        'com_cliente_id' => 'required',
        'com_org_id' => 'required',
        'codigo' => 'required|min:2|max:10',
        'nombre' => 'required|min:2|max:100',
        'id_legal' => 'min:2|max:20',
        'siglas' => 'min:2|max:20',
        'pro_actividad_proveedor_id' => 'required'
    ];

    /**
     * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
     **/
    public function cliente()
    {
        return $this->belongsTo(\InnovaCondomi\Entities\Com\Cliente::class, 'com_cliente_id', 'id');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
     **/
    public function org()
    {
        return $this->belongsTo(\InnovaCondomi\Entities\Com\Org::class, 'com_org_id', 'id');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\BelongsTo
     **/
    public function actividadProveedor()
    {
        return $this->belongsTo(\InnovaCondomi\Entities\Pro\ActividadProveedor::class, 'pro_actividad_proveedor_id', 'id');
    }

    /**
    * Obtener todas las Direcciones para el Modelo.
    */
    public function direcciones()
    {
        return $this->morphMany('InnovaCondomi\Entities\Com\Direccion','direccionable');
    }

    /**
     * Obtener todas las Direcciones para el Modelo.
     */
    public function telefonos()
    {
        return $this->morphMany('InnovaCondomi\Entities\Com\Telefono','telefoneable');
    }

    /**
     * Obtener todas los Contactos Web para el Modelo.
     */
    public function contactosWeb()
    {
        return $this->morphMany('InnovaCondomi\Entities\Com\ContactoWeb','contactable');
    }
}

...
class MonedaDataTable extends DataTable
{
    ...
    public function ajax()
    {
        return $this->datatables
            ->eloquent($this->query())
            ->addColumn('action', 'com.monedas.datatables_actions')
            ->editColumn('activo', function (Moneda $row) {
    ...
    public function query()
    {
        ...
        $monedas = Moneda::select('com_moneda.*', 'com_cliente.nombre as nombre_cliente', 'com_org.nombre as nombre_org')
            ->join('com_org', 'com_org.id', '=', 'com_moneda.com_org_id')
            ->join('com_cliente', 'com_cliente.id', '=', 'com_moneda.com_cliente_id');

        return $this->applyScopes($monedas);
    }
...
//                'dom' => 'lfrtip',
    ...
    private function getColumns()
    {
        return [
            ...
            'codigo_iso' => ['name' => 'codigo_iso', 'data' => 'codigo_iso', 'title' => 'Código ISO'],
            'nombre' => ['name' => 'nombre', 'data' => 'nombre', 'title' => 'Nombre'],
            'simbolo' => ['name' => 'simbolo', 'data' => 'simbolo', 'title' => 'Símbolo'],
            'activo'  => ['name' => 'com_moneda.activo', 'data' => 'activo',

...
class RolDataTable extends DataTable
{
    ...
    public function ajax()
    {
        return $this->datatables
            ->eloquent($this->query())
            ->addColumn('action', 'seg.rols.datatables_actions')
            ->make(true);
    }
    ...
    {
        //$rols = Rol::query();
        $rols = Rol::select('seg_rol.*', 'com_cliente.nombre as nombre_cliente', 'com_org.nombre as nombre_org')
            ->join('com_org', 'com_org.id', '=', 'seg_rol.com_org_id')
            ->join('com_cliente', 'com_cliente.id', '=', 'seg_rol.com_cliente_id');

        return $this->applyScopes($rols);
    }
...
//                'dom' => 'lfrtip',
    ...
    private function getColumns()
    {
        return [
            ...
            'nivel_jerarquia' => ['name' => 'nivel_jerarquia', 'data' => 'nivel_jerarquia'],
            'nivel_acceso' => ['name' => 'nivel_acceso', 'data' => 'nivel_acceso'],
